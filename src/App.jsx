...
const PLANS = [
  { name: "ANALYST", planKey: "analyst", price: "$29", per: "/mo", tagline: "For individual strategists", items: ["5 reports per month", "Copy & export", "6-month archive", "Email support"], cta: "Start Free Trial", hot: false },
  { name: "INTELLIGENCE", planKey: "intelligence", price: "$79", per: "/mo", tagline: "For teams & operators", items: ["Unlimited reports", "API access", "Custom branding", "Priority generation", "Team dashboard"], cta: "Go Intelligence →", hot: true },
  { name: "ENTERPRISE", planKey: "enterprise", price: "$249", per: "/mo", tagline: "For agencies & funds", items: ["Unlimited everything", "White-label output", "10 team seats", "Webhook delivery", "Dedicated support"], cta: "Contact Sales", hot: false },
];
...
export default function ReportForge() {
  ...
  const openCheckout = async (plan) => {
    ...
    try {
      const res = await fetch(`/api/checkout?plan=${plan}&email=${encodeURIComponent(userEmail)}`);
      const data = await res.json();
      if (data.url) window.open(data.url, "_blank", "noopener");
    } catch {
      // fallback: go to store homepage
      window.open("https://app.lemonsqueezy.com", "_blank", "noopener");
    } finally {
      setCheckoutLoading("");
    }
  };
        ...
        <div className="rf-plans">
          {PLANS.map(p => (
            <div key={p.name} className={`rf-plan${p.hot?" hot":""}`}>
              ...
              <button
                className="rf-plan-btn"
                onClick={() => openCheckout(p.planKey)}
                disabled={checkoutLoading === p.planKey}
              >
                {checkoutLoading === p.planKey ? "Opening..." : p.cta}
              </button>
            </div>
          ))}
        </div>
    ...
    return (
      <div className="rf-rep-wrap">
        ...
        <div className="rf-upsell">
          ...
          <button className="rf-btn-gold" style={{margin:"0 auto"}} onClick={() => openCheckout("intelligence")}>Upgrade Now →</button>
        </div>
